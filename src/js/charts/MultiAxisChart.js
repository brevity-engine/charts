...
export default class MultiAxisChart extends AxisChart {
  ...
  setMeasures() {
    super.setMeasures();
    const noOfLeftAxes = this.data.datasets.filter(
  ...
  prepareData(data) {
    ...
    const sets = this.state.datasets;
  ...
  getYAxesComponents() {
    return this.data.datasets.map((e, i) => {
      return new ChartComponent({
        ...
        make: () => {
          const yAxis = this.state.datasets[i].yAxis;
  ...
  getChartComponents() {
    return this.data.datasets.map((d, index) => {
      return new ChartComponent({
        ...
        make: () => {
          const d = this.state.datasets[index];
          const unitType = this.unitArgs;
        ...
        animate: (svgUnits) => {
          const d = this.state.datasets[index];
          const unitType = this.unitArgs.type;

          // have been updated in axis render;
          const newX = this.state.xAxisPositions;
          const newY = this.state.datasets[index].positions;

          const lastUnit = svgUnits[svgUnits.length - 1];
          const parentNode = lastUnit.parentNode;

          if (this.oldState.xExtra > 0) {
            for (let i = 0; i < this.oldState.xExtra; i++) {
              const unit = lastUnit.cloneNode(true);
